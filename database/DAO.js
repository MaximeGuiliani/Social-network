...
//SQLite
// SELECT Events.id, Events.participants_number, Events.category, Events.address, Events.description, Events.image_url, Events.name, Events.organizerId, count(*) as nb_participants, Events.participants_number - count(*) as nb_places_left, USers.score_host, Users.score_participant, Users.username, Users.email
// FROM Events
// JOIN EventParticipants ON Events.id = EventParticipants.EventId JOIN Users ON Events.organizerId = Users.id
// GROUP BY Events.id
// HAVING nb_places_left>42
//     AND Events.category LIKE '%Randonne%'
//     AND Events.participants_number BETWEEN 0 AND 60
//     AND Events.description LIKE '%rando%'
//     AND Events.name LIKE '%Luminy%'
//     AND (Events.address LIKE '%zz%' OR Events.address LIKE '%adresse%')
//     AND Users.username LIKE '%paul%';


// having: {
// 	[Op.and]: [
// 	  this.sequelize.where(this.sequelize.fn('LOWER', this.sequelize.col('category')), Op.like, `%${category}%`),
// 	  { participants_number: {[Op.between]: [range_places.min, range_places.max]} },
// 	  { description: {[Op.like]: `%${description}%`} },
// 	  { name: {[Op.like]: `%${event_name}%`} },
// 	  { address: {[Op.like]: `%${address}%`} },
// 	  { 'organizer.username': {[Op.like]: `%${username}%`} },
// 	  { 'nb_places_left': {[Op.gte]: nb_places_wanted} },
// 	  { date : {
// 		[Op.lte]: range_date.max,
// 		[Op.gte]: range_date.min,
// 	  }},
// 	]}
// });
