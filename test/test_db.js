...
describe("TEST DB", () => {
  describe("TEST DAO", () => {
    ...
    it("test create user", async () => {
      ...
      expect(user.password_hash).to.equal(
        "$2b$10$3uDKF58gsus/Bd/0Z.ZI5elFdfr.emhTRVs6U3924qm59Spuaojbm"
      );
    });
    ...
    it("test applying to event", async () => {
      ...
      const user_with_events = await myDAO.get_user_with_related_events({
        id: user.id,
      ...
      expect(user_with_events.candidateEvents[0].name).to.equal(
        "Jazz Convergence"
      );
    });
    ...
    it("test search - date not found ", async () => {
      const events = await myDAO.get_events_by_filters({
        range_date: {
          min: "2023-04-21T13:31:00.000Z",
          max: "2023-04-21T13:32:00.000Z",
        },
    ...
    it("test search - main category found ", async () => {
      const cat_jv = await myDAO.get_main_category_by_name("Jeux-Vidéos");
      const events = await myDAO.get_events_by_filters({
        MainCategoryId: 999,
        range_date: {
          min: "2023-04-21T13:30:00.000Z",
    ...
    it("get event with related users", async () => {
      ...
      let event_with_related_users = await myDAO.get_event_with_related_users({
        eventId: my_event.id,
    ...
    it("test note_host_is_possible OK", async () => {
      ...
      const is_possilbe = await myDAO.note_host_is_possible({
        ownerId: hugo.id,
        targetId: franck_host_jazz.id,
        eventId: jazz.id,
      });
    ...
    it("test note_participant_is_possible OK", async () => {
      ...
      const is_possilbe = await myDAO.note_participant_is_possible({
        ownerId: franck_host_jazz.id,
        targetId: hugo.id,
        eventId: jazz.id,
      });
      expect(is_possilbe).to.equal(true);
    ...
    it("test get_user_with_related_events", async () => {
      ...
      const user = await myDAO.get_user_with_related_events({
        id: franck.id,
